...
static RobotPC_Recv_s *pc_recv_data; // 上位机接收数据指针,初始化时返回
static RobotPC_Send_s pc_send_data;  // 上位机发送数据
...
void RobotCMDInit()
{
    robot_config = RobotConfigInit(INFANTRY_ROBOT_4);
    ...
    pc_recv_data = RobotPCInit(&huart1); // 视觉通信串口
...
static void RemoteControlSet()
{
    ...
    add_pitch = 0.001f * (float)rc_data[TEMP].rc.rocker_l1;
    ...
    // { 
        ...
        { // 按照摇杆的输出大小进行角度增量,拨杆向右/向上为正 
            ...
            if (pc_recv_data->target_state == TARGET_CONVERGING)
            {
                gimbal_cmd_send.yaw = -pc_recv_data->vision_data.yaw;
                gimbal_cmd_send.pitch = pc_recv_data->vision_data.pitch;  

                add_yaw = -pc_recv_data->vision_data.yaw;
                add_pitch = pc_recv_data->vision_data.pitch;    
            } 
        }
    // }
    ...
    chassis_cmd_send.vx = 0.2f / HALF_RC_CH_MAX * (float)rc_data[TEMP].rc.rocker_r_;    // 横移方向
    chassis_cmd_send.wz = 0.1f / HALF_RC_CH_MAX * (float)rc_data[TEMP].rc.rocker_l_;  

    // 底盘imu数据
    chassis_cmd_send.pitch = gimbal_fetch_data.gimbal_imu_data.Pitch * DEGREE_2_RAD;
    chassis_cmd_send.yaw = gimbal_fetch_data.gimbal_imu_data.Yaw * DEGREE_2_RAD;
    chassis_cmd_send.pitch_gyro = gimbal_fetch_data.gimbal_imu_data.Gyro[0];
    chassis_cmd_send.yaw_gyro = gimbal_fetch_data.gimbal_imu_data.Gyro[2];

    chassis_cmd_send.accel = gimbal_fetch_data.gimbal_imu_data.Accel[1];
    // chassis_cmd_send.vx = 1.5f / HALF_RC_CH_MAX * (float)rc_data[TEMP].rc.rocker_r_;    // 横移方向
...
void RobotCMDTask()
{
    ...
    pc_send_data.enemy_color = referee_data->GameRobotState.robot_id > 7 ? 0 : 1;
